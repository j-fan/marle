<script lang="ts">
  ...
  const drawSquare = (x0: number, y0: number, size: number, pattern: number, operatorIndex: number, steps: number) => {
    ctx.save();
    ...
    for(let col = 0; col < steps; col++) {
      ...
      for(let row = 0; row < steps; row++) {
        const patternRowPart = patternBinaryString[row + steps] as unknown as number;
